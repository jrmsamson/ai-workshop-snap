...
import { discoverTests } from "./yaml-loader.js";
import { runCase } from "./runner.js";
import { reportResult, reportSummary, summaryText } from "./reporter.js";
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    switch (argv[i]) {
      case "--candidate":
        result.candidate = required(argv, ++i, "--candidate");
        break;
      case "--tests":
        result.tests = resolve(callerCwd, required(argv, ++i, "--tests"));
        break;
      case "--filter":
        result.filter = required(argv, ++i, "--filter");
        break;
      case "--summary":
        result.summary = resolve(callerCwd, required(argv, ++i, "--summary"));
        break;
      case "--verbose":
      case "-v":
        result.verbose = true;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    throw new Error(`candidate is not executable: ${candidate}`);
  }

  process.stdout.write(`snap tests — candidate=${candidate}, ${String(tests.length)} case(s)\n`);
  const results = [];
...
function usage(code: number): never {
  process.stdout.write(
    `Usage: verify [--candidate PATH] [options]\n\n` +
