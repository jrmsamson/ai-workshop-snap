...
import { validateVariableName } from "./interpolate.js";
import type {
  CaptureValue,
  Environment,
  ExpectedTreeEntry,
...
export function discoverTests(testsDir: string, filter?: string): TestCase[] {
  ...
  return tests.filter(
    (test) => basename(test.source).includes(filter) || test.name.includes(filter),
  );
}
...
export function loadTest(path: string): TestCase {
  ...
  return {
    ...
    ...(root["description"] === undefined
      ? {}
      : { description: string(root["description"], `${path}.description`) }),
    ...(root["timeout"] === undefined
      ? {}
      : { timeout: positive(root["timeout"], `${path}.timeout`) }),
    ...(root["env"] === undefined ? {} : { env: environment(root["env"], `${path}.env`) }),
    steps: steps.map((step, index) => parseStep(step, `${path}.steps[${String(index)}]`)),
  };
}

function parseStep(value: unknown, location: string): Step {
  const outer = mapping(value, location);
  const entries = Object.entries(outer);
  const first = entries[0];
  if (entries.length !== 1 || first === undefined)
    throw new Error(`${location}: expected exactly one operation key`);
  const [tag, payload] = first;
  switch (tag) {
    case "mkdir":
      return { type: "mkdir", path: pathPayload(payload, location) };
    case "remove":
      return { type: "remove", path: pathPayload(payload, location) };
    case "fifo":
      return { type: "fifo", path: pathPayload(payload, location) };
    case "write_file": {
      ...
      const base = {
        ...
        path: string(map["path"], `${location}.write_file.path`),
      };
    ...
    case "copy_tree": {
      ...
      return {
        type: "copy_tree",
        from: string(map["from"], `${location}.copy_tree.from`),
        to: string(map["to"], `${location}.copy_tree.to`),
      };
    }
    case "symlink": {
      ...
      return {
        type: "symlink",
        path: string(map["path"], `${location}.symlink.path`),
        target: string(map["target"], `${location}.symlink.target`),
      };
    }
    case "run":
      return { type: "run", value: runStep(payload, `${location}.run`) };
    case "start":
      return { type: "start", value: startStep(payload, `${location}.start`) };
    case "stop":
      return { type: "stop", value: stopStep(payload, `${location}.stop`) };
    case "start_http":
      return { type: "start_http", value: startHttpStep(payload, `${location}.start_http`) };
    case "stop_http": {
    ...
    case "http_request":
      return { type: "http_request", value: httpRequestStep(payload, `${location}.http_request`) };
    case "assert": {
      const values = array(payload, `${location}.assert`);
      if (values.length === 0) throw new Error(`${location}.assert: must not be empty`);
      return {
        type: "assert",
        assertions: values.map((item, i) =>
          stateAssertion(item, `${location}.assert[${String(i)}]`),
        ),
      };
    }
    default:
      throw new Error(`${location}: unknown operation: ${tag}`);
  }
}
...
function runStep(value: unknown, location: string): RunStep {
  ...
  return {
    ...
    ...(map["timeout"] === undefined
      ? {}
      : { timeout: positive(map["timeout"], `${location}.timeout`) }),
    ...(map["env"] === undefined ? {} : { env: environment(map["env"], `${location}.env`) }),
    ...(map["capture"] === undefined
      ? {}
      : { capture: captureMap(map["capture"], `${location}.capture`) }),
    expect,
  };
}
...
function startStep(value: unknown, location: string): StartStep {
  ...
  const stream = enumValue(
    ready["stream"],
    ["stdout", "stderr"] as const,
    `${location}.ready.stream`,
  );
  const pattern = regex(ready["pattern"], `${location}.ready.pattern`);
  let capture: StartStep["ready"]["capture"];
  if (ready["capture"] !== undefined) {
    const captureConfig = mapping(ready["capture"], `${location}.ready.capture`);
    exact(captureConfig, ["as", "group"], `${location}.ready.capture`);
    const as = identifier(captureConfig["as"], `${location}.ready.capture.as`);
    validateVariableName(as, `${location}.ready.capture.as`);
    capture = {
      as,
      ...(captureConfig["group"] === undefined
        ? {}
        : { group: nonnegative(captureConfig["group"], `${location}.ready.capture.group`) }),
    };
  }
  return {
...
function stopStep(value: unknown, location: string): StopStep {
  ...
  return {
    ...
    ...(map["signal"] === undefined
      ? {}
      : { signal: enumValue(map["signal"], ["SIGTERM", "SIGINT"] as const, `${location}.signal`) }),
    ...(map["timeout"] === undefined
      ? {}
      : { timeout: positive(map["timeout"], `${location}.timeout`) }),
...
function startHttpStep(value: unknown, location: string): StartHttpStep {
  ...
  return {
    ...
    routes: array(map["routes"], `${location}.routes`).map((route, i) =>
      httpRoute(route, `${location}.routes[${String(i)}]`),
    ),
  };
}
...
function httpRoute(value: unknown, location: string): HttpRoute {
  ...
  if (map["text"] !== undefined && map["base64"] !== undefined)
    throw new Error(`${location}: text and base64 are mutually exclusive`);
  return {
    ...
    ...(map["headers"] === undefined
      ? {}
      : { headers: stringMap(map["headers"], `${location}.headers`) }),
    ...(map["text"] === undefined ? {} : { text: string(map["text"], `${location}.text`) }),
    ...(map["base64"] === undefined
      ? {}
      : { base64: base64Template(map["base64"], `${location}.base64`) }),
  };
}
...
function httpRequestStep(value: unknown, location: string): HttpRequestStep {
  ...
  return {
    ...
    ...(map["timeout"] === undefined
      ? {}
      : { timeout: positive(map["timeout"], `${location}.timeout`) }),
    expect: expects.map((item, i) => httpAssertion(item, `${location}.expect[${String(i)}]`)),
  };
}
...
function processAssertions(
  ...
  const assertions = values.map((item, index) =>
    processAssertion(item, `${location}[${String(index)}]`),
  );
  const exits = assertions.filter((assertion) => assertion.type === "exit_code").length;
  if (requireExit && exits !== 1)
    throw new Error(`${location}: requires exactly one exit_code assertion`);
  return assertions;
}
...
function httpAssertion(value: unknown, location: string): HttpAssertion {
  ...
  if (type === "header_equals") {
    ...
    return {
      type,
      name: string(map["name"], `${location}.name`),
      value: string(map["value"], `${location}.value`),
    };
  }
    ...
    return {
      ...
      value:
        type === "body_base64_equals"
          ? base64Template(map["value"], `${location}.value`)
          : string(map["value"], `${location}.value`),
    };
  }
...
function stateAssertion(value: unknown, location: string): StateAssertion {
  ...
  switch (type) {
    case "tree_equals": {
      ...
      const entries = array(map["entries"], `${location}.entries`).map((entry, i) =>
        treeEntry(entry, `${location}.entries[${String(i)}]`),
      );
      const sorted = [...entries].sort((a, b) =>
        Buffer.compare(Buffer.from(a.path), Buffer.from(b.path)),
      );
      if (!isSameOrder(entries, sorted))
        throw new Error(`${location}.entries: must be in unsigned UTF-8 path order`);
    ...
    case "file_base64_equals": {
      ...
      return {
        type,
        path: string(map["path"], `${location}.path`),
        value:
      ...
      return {
        ...
        ...(map["kind"] === undefined
          ...
          : {
              kind: enumValue(
                map["kind"],
                ["file", "directory", "symlink", "fifo", "other"] as const,
                `${location}.kind`,
              ),
            }),
      };
    ...
    case "trees_equal":
      ...
      return {
        type,
        left: string(map["left"], `${location}.left`),
        right: string(map["right"], `${location}.right`),
        ...(map["ignore"] === undefined
          ? {}
          : { ignore: strings(map["ignore"], `${location}.ignore`) }),
      };
    case "http_requests_equal": {
      exact(map, ["type", "server", "value"], location);
      const requests = array(map["value"], `${location}.value`).map((item, i) => {
        const request = mapping(item, `${location}.value[${String(i)}]`);
        exact(request, ["method", "target"], `${location}.value[${String(i)}]`);
        return {
          method: method(request["method"], `${location}.value[${String(i)}].method`),
          target: string(request["target"], `${location}.value[${String(i)}].target`),
        };
      });
      return { type, server: identifier(map["server"], `${location}.server`), value: requests };
    }
    default:
      throw new Error(`${location}.type: unknown state assertion: ${type}`);
  }
}
...
function treeEntry(value: unknown, location: string): ExpectedTreeEntry {
  ...
  if (kind === "symlink" && map["target"] === undefined)
    throw new Error(`${location}.target: required for symlink`);
  if (kind !== "symlink" && map["target"] !== undefined)
    throw new Error(`${location}.target: only valid for symlink`);
  return {
    path: string(map["path"], `${location}.path`),
    kind,
    ...(map["target"] === undefined ? {} : { target: string(map["target"], `${location}.target`) }),
  };
}

function captureMap(value: unknown, location: string): NonNullable<RunStep["capture"]> {
  const map = mapping(value, location);
  exact(map, ["stdout", "stderr"], location);
  if (map["stdout"] === undefined && map["stderr"] === undefined)
    throw new Error(`${location}: must capture stdout or stderr`);
  return {
    ...(map["stdout"] === undefined
      ? {}
      : { stdout: captureValue(map["stdout"], `${location}.stdout`) }),
    ...(map["stderr"] === undefined
      ? {}
      : { stderr: captureValue(map["stderr"], `${location}.stderr`) }),
  };
}
...
function captureValue(value: unknown, location: string): CaptureValue {
  ...
  return {
    as,
    ...(map["trim"] === undefined ? {} : { trim: boolean(map["trim"], `${location}.trim`) }),
  };
}
  ...
  return Object.fromEntries(
    Object.entries(map).map(([key, item]) => {
      if (item !== null && typeof item !== "string")
        throw new Error(`${location}.${key}: expected string or null`);
      return [key, item];
    }),
  );
}
...
function stringMap(value: unknown, location: string): Record<string, string> {
  const map = mapping(value, location);
  return Object.fromEntries(
    Object.entries(map).map(([key, item]) => [key, string(item, `${location}.${key}`)]),
  );
}

function exclusive(map: MapValue, a: string, b: string, location: string): void {
  if ((map[a] === undefined) === (map[b] === undefined))
    throw new Error(`${location}: exactly one of ${a} and ${b} is required`);
}
...
function mapping(value: unknown, location: string): MapValue {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new Error(`${location}: expected mapping`);
...
function strings(value: unknown, location: string): string[] {
  return array(value, location).map((item, i) => string(item, `${location}[${String(i)}]`));
}
...
function identifier(value: unknown, location: string): string {
  ...
  if (!/^[A-Za-z_][A-Za-z0-9_-]*$/.test(result))
    throw new Error(`${location}: invalid identifier: ${result}`);
  return result;
}
...
function regex(value: unknown, location: string): string {
  ...
  try {
    ...
    throw new Error(`${location}: invalid regex: ${(error as Error).message}`);
  }
  return result;
}
